...
async function iniciarSesionUsuario(e) {
    ...
    // 1. Verificación universal para usuario Admin Master
    ...
    // 2. Traer la lista actualizada directamente del backend antes de validar
    try {
        const resUsr = await fetch('/api/usuarios');
        usuarios = await resUsr.json();
    } catch (err) {
        console.error("Error al consultar usuarios desde el servidor:", err);
    }

    // 3. Verificación de usuarios registrados
    const usrObj = usuarios.find(u => 
        u.nombre.trim().toLowerCase() === usrName.toLowerCase() && 
